    ...
    def react(self, message):
        ...
        if message.performative == ACLMessage.PROPOSE:
            ...
            if name == "Book_store":
                ...
                message = ACLMessage()
                message.set_performative(ACLMessage.ACCEPT_PROPOSAL)
                # отправляем сообщение с тем, что хочет клиент, параметр name и цену
                message.set_content(json.dumps({'genre': self.i_want[number], 'name': "client", 'price': 0}))
                message.add_receiver(AID(name="agent_book_store@localhost:8022"))
                self.send(message)
            elif name == "seller":
                ...
                message.set_performative(ACLMessage.ACCEPT_PROPOSAL)
                money = 1000 - self.money
                message.set_content(json.dumps({'talk': "talk1"}))
                message.add_receiver(AID(name="agent_book_store@localhost:8033"))
                self.send(message)
        elif message.performative == ACLMessage.ACCEPT_PROPOSAL:
            content = json.loads(message.content)
            flag = content['flag']
...
#  создаем класс для воторого агента
class Book_store(Agent):
    def __init__(self, aid):
        super(Book_store, self).__init__(aid=aid, debug=False)
    ...
    def react(self, message):
        ...
        if message.performative == ACLMessage.PROPOSE:
            ...
            message = ACLMessage()
            message.set_performative(ACLMessage.PROPOSE)
            # отправляем значение Book_store для аргумента name
            message.set_content(json.dumps({'name': 'Book_store'}))
            message.add_receiver(AID(name="agent_client@localhost:8011"))
            self.send(message)
        elif message.performative == ACLMessage.ACCEPT_PROPOSAL:
            ...
            if price == 0:
                ...
                message.set_performative(ACLMessage.ACCEPT_PROPOSAL)
                message.set_content(
                    json.dumps({'type': format(genre), 'price': self.menu[self.current_book][0], 'flag': 0}))
                message.add_receiver(AID(name="agent_client@localhost:8011"))
                self.send(message)
            else:
                display_message(self.aid.localname, "Спасибо за покупку, хотите выбрать другую книгу?")
            ...
            if cause == 'discount':
                ...
                display_message(self.aid.localname, "Привет, у меня есть время поболтать))")
                message = ACLMessage()
                message.set_performative(ACLMessage.PROPOSE)
                message.set_content(json.dumps({'talk ': "talk2"}))
                message.add_receiver(AID(name="agent_client@localhost:8033"))
                self.send(message)
            elif cause == 'talk23':
                display_message(self.aid.localname, "Так себе, только один клинет")
                message = ACLMessage()
                message.set_performative(ACLMessage.PROPOSE)
                message.set_content(json.dumps({'talk ': "talk3"}))
                message.add_receiver(AID(name="agent_client@localhost:8033"))
                self.send(message)
            elif cause == 'talk34':
                display_message(self.aid.localname, "Спасибо!")
                display_message(self.aid.localname, "Пулучил {}".format(game))
                message = ACLMessage()
                message.set_performative(ACLMessage.PROPOSE)
                message.set_content(json.dumps({'talk ': "talk4"}))
                message.add_receiver(AID(name="agent_client@localhost:8033"))
                self.send(message)
...
class New_client:
    def __init__(self, aid):
        super(New_client, self).__init__(aid=aid, debug=False)
        self.game = 'Сверический конь в вакууме'
    def react(self, message):
        super(New_client, self).react(message)
        if message.performative == ACLMessage.PROPOSE:
            content = json.loads(message.content)
            talk = content['talk']
            if talk == 'talk1':
                display_message(self.aid.localname, "Привет, я пришел поговорить!")
                message = ACLMessage()
                message.set_performative(ACLMessage.PROPOSE)
                message.set_content(json.dumps({'cause': "talk12"}))
                message.add_receiver(AID(name="agent_client@localhost:8022"))
                self.send(message)
            elif talk == 'talk2':
                display_message(self.aid.localname, "Как дела?")
                message = ACLMessage()
                message.set_performative(ACLMessage.PROPOSE)
                message.set_content(json.dumps({'cause': "talk12"}))
                message.add_receiver(AID(name="agent_client@localhost:8022"))
                ...
                display_message(self.aid.localname, "Тебе скушно? На посмотри")
                message = ACLMessage()
                message.set_performative(ACLMessage.PROPOSE)
                message.set_content(json.dumps({'cause': "talk12", 'game':self.game}))
                message.add_receiver(AID(name="agent_client@localhost:8022"))
                self.send(message)
            elif talk == 'talk4':
                display_message(self.aid.localname, "Пожалуйста")
...
if __name__ == '__main__':
    ...
    agent_client = Client(AID(name="agent_client@localhost:8011"))
    agent_book_store = Book_store(AID(name="agent_book_store@localhost:8022"))
    agent_new_client = New_client(AID(name="agent_book_store@localhost:8033"))

    agents.append(agent_client)
    agents.append(agent_book_store)
    agents.append(New_client)
